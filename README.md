# DN 5.0 Deep Skilling - Java FSE React

## Candidate Details
- **Name**: Saisameer Pamidighantam
- **Program**: Cognizant Digital Nurture 5.0
- **Track**: Java FSE React
- **Institute**: SRM Institute of Science and Technology

## Repository Structure

| Week   | Modules                                                        |
|--------|----------------------------------------------------------------|
| Week 1 | Design Patterns & DSA                                          |
| Week 2 | PL/SQL, TDD, JUnit5, Mockito, SLF4J                           |
| Week 3 | Spring Core, Maven, Spring Data JPA, Hibernate                 |
| Week 4 | Spring REST using Spring Boot 3                                |
| Week 5 | Microservices, Spring Cloud, React (start)                     |
| Week 6 | React (complete), Application Debugging                        |
| Week 7 | Git, DevOps, Docker, Agile, Cloud, GenAI                       |

## Mandatory Exercises Tracker

### Week 1 - Design Patterns & DSA
- [x] Module 1 - Singleton Pattern
- [x] Module 1 - Factory Method Pattern
- [x] Module 2 - E-commerce Search Function
- [x] Module 2 - Financial Forecasting

### Week 2 - PL/SQL, TDD, JUnit5, Mockito, SLF4J
- [x] Module 3 - Control Structures (PL/SQL)
- [x] Module 3 - Stored Procedures
- [x] Module 4 - Setting Up JUnit
- [x] Module 4 - Assertions in JUnit
- [x] Module 4 - AAA Pattern
- [x] Module 4 - Mocking and Stubbing
- [x] Module 4 - Verifying Interactions
- [x] Module 4 - SLF4J Logging

### Week 3 - Spring Core & Spring Data JPA
- [x] Module 5 - Basic Spring Application
- [x] Module 5 - Dependency Injection
- [x] Module 5 - Maven Project
- [x] Module 6 - Spring Data JPA Quick Example

### Week 4 - Spring REST
- [x] Module 7 - Spring Web Project
- [x] Module 7 - Hello World REST
- [x] Module 7 - Country Web Service
- [x] Module 7 - JWT Authentication

### Week 5 - Microservices & React
- [ ] Module 8 - Microservices (Account & Loan)
- [ ] Module 9 - React HOL 1-4

### Week 6 - React 
- [ ] Module 9 - React HOL 5, 9-13

### Week 7 - Platforms & GenAI
- [ ] Module 11 - Git HOL 1-5
